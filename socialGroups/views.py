...
class JoinGroup(LoginRequiredMixin, generic.RedirectView):
    ...
    def get(self, request, *args, **kwargs):
        group = get_object_or_404(models.SocialGroup, slug=self.kwargs.get('slug'))

        try:
            models.SocialGroupMembers.objects.create(user=self.request.user, group=group)
...
class LeaveGroup(LoginRequiredMixin, generic.RedirectView):
    ...
    def get(self, request, *args, **kwargs):
        try:
            membership = models.SocialGroupMembers.objects.filter(
                user=self.request.user,
                group__slug=self.kwargs.get('slug')
            )
        except models.SocialGroupMembers.DoesNotExist:
